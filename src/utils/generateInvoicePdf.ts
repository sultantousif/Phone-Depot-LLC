...
export function generateInvoicePdf({
  ...
  const termsText = [
    ...
    '• Approved & processed under HG World Class Master Commercial Wholesale Agreement.',
  ];
  ...
  if (invoice.notes) {
    ...
    termsText.push('• Thank you for partnering with HG World Class Wholesale Distribution.');
  }
  ...
  doc.text(
    `Generated by HG World Class Wholesale Portal on ${new Date().toLocaleString()} | Page 1 of 1`,
    margin,
    footerY + 14
  );
...
export function generatePaymentInvoicePdf({
  ...
  creditLineText.push('• HG World Class Commercial Wholesale Portal record.');
  ...
  doc.setFontSize(7);
  doc.setTextColor(71, 85, 105);
  doc.text('HG WORLD CLASS WHOLESALE AUTHORIZED SIGNATURE', rightBoxX + 10, summaryY + 84);
  ...
  doc.setTextColor(148, 163, 184);
  doc.text(
    `Payment Invoice & Settlement Voucher | Generated on ${new Date().toLocaleString()} | HG World Class Portal`,
    margin,
    footerY + 14
  );
